...
open class TapTitanCommandControllerMix2S(protected val tapTitanViewModel: TapTitanViewModel) :
    ...
    override suspend fun reborn(closePanel: Boolean) {
        println("reborn")
        if(closePanel) {
            closePanel()
        }
    ...
    override suspend fun upgrade(closePanel: Boolean) {
        if (closePanel) {
            closePanel()
        }
    ...
    protected suspend fun autoUpgrade(closePanel: Boolean = true) {
        if (closePanel) {
            closePanel()
        }
        command {
    ...
    protected suspend fun
        ...
        command {
            ...
            delay(if (tapTitanViewModel.realStop.value) 10000 else 5000)
        }
    }
